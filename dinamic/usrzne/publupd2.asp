...
if (String(Request("MM_update")) != "undefined" &&
  ...
  var MM_editTable  = "public";
  var MM_editColumn = "datosID";
  var MM_recordId = "" + Request.Form("MM_recordId") + "";
  var MM_editRedirectUrl = "publupdok.asp";
  var MM_fieldsStr = "select|value|hiddenField|value|txt_titulo|value|txt_datos|value|txt_year|value";
  var MM_columnsStr = "tipoPublicID|none,none,NULL|integID|none,none,NULL|tituloPublic|',none,''|datosPublic|',none,''|yearPublic|none,none,NULL";
...
%>
<%
var rsAldea21PublUpdTP = Server.CreateObject("ADODB.Recordset");
rsAldea21PublUpdTP.ActiveConnection = MM_connaldea21_STRING;
rsAldea21PublUpdTP.Source = "SELECT * FROM publicTipo";
rsAldea21PublUpdTP.CursorType = 0;
rsAldea21PublUpdTP.CursorLocation = 2;
rsAldea21PublUpdTP.LockType = 3;
rsAldea21PublUpdTP.Open();
var rsAldea21PublUpdTP_numRows = 0;
%>
<%
var rsAldea21PublUpd__MMColParam = "1";
if(String(Request.QueryString("datosID")) != "undefined") { 
  rsAldea21PublUpd__MMColParam = String(Request.QueryString("datosID"));
}
%>
<%
var rsAldea21PublUpd = Server.CreateObject("ADODB.Recordset");
rsAldea21PublUpd.ActiveConnection = MM_connaldea21_STRING;
rsAldea21PublUpd.Source = "SELECT * FROM public WHERE datosID = "+ rsAldea21PublUpd__MMColParam.replace(/'/g, "''") + "";
rsAldea21PublUpd.CursorType = 0;
rsAldea21PublUpd.CursorLocation = 2;
rsAldea21PublUpd.LockType = 3;
rsAldea21PublUpd.Open();
var rsAldea21PublUpd_numRows = 0;
%>
<html>
<!-- #BeginTemplate "/Templates/model.dwt" --> 
<head>
<!-- #BeginEditable "doctitle" --> 
<title>PPI Aldea 21 - Actualizar publicaci&oacute;n - 2 de 3</title>
<!-- #EndEditable --> 
        ...
        <tr> 
          <td> 
            <p><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><b>ACTUALIZAR 
              PUBLICACI&Oacute;N - 2 de 3</b></font></p>
            <p><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><i>Actualice 
              los datos</i></font></p>
            <form name="form1" method="POST" action="<%=MM_editAction%>">
              <p>&nbsp; </p>
              <table width="100%" border="1">
                <tr> 
                  <td bgcolor="#FFCC99"><i><b><font face="Verdana, Arial, Helvetica, sans-serif" size="1">Tipo 
                    de publicaci&oacute;n</font></b></i></td>
                  <td> <font face="Verdana, Arial, Helvetica, sans-serif" size="2"> 
                    <select name="select">
                      <% 
while (!rsAldea21PublUpdTP.EOF) {
%>
                      <option value="<%=(rsAldea21PublUpdTP.Fields.Item("tipoPublicID").Value)%>" <%=((rsAldea21PublUpdTP.Fields.Item("tipoPublicID").Value == rsAldea21PublUpd.Fields.Item("tipoPublicID").Value)?"SELECTED":"")%>><%=(rsAldea21PublUpdTP.Fields.Item("tipoPublic").Value)%></option>
                      <%
  rsAldea21PublUpdTP.MoveNext();
}
if (rsAldea21PublUpdTP.CursorType > 0) {
  if (!rsAldea21PublUpdTP.BOF) rsAldea21PublUpdTP.MoveFirst();
} else {
  rsAldea21PublUpdTP.Requery();
}
%>
                    </select>
                    <input type="hidden" name="hiddenField" value="<%=(rsAldea21PublUpd.Fields.Item("integID").Value)%>">
                    </font></td>
                </tr>
                <tr> 
                  <td bgcolor="#FFCC99"><i><b><font face="Verdana, Arial, Helvetica, sans-serif" size="1">T&iacute;tulo 
                    de la publicaci&oacute;n</font></b></i></td>
                  <td> <font face="Verdana, Arial, Helvetica, sans-serif" size="2"> 
                    <textarea name="txt_titulo" cols="100" rows="2"><%=(rsAldea21PublUpd.Fields.Item("tituloPublic").Value)%></textarea>
                    </font></td>
                </tr>
                <tr> 
                  <td bgcolor="#FFCC99"><i><b><font face="Verdana, Arial, Helvetica, sans-serif" size="1">Datos 
                    de la publicaci&oacute;n</font></b></i></td>
                  <td> <font face="Verdana, Arial, Helvetica, sans-serif" size="2"> 
                    <textarea name="txt_datos" cols="100" rows="4"><%=(rsAldea21PublUpd.Fields.Item("datosPublic").Value)%></textarea>
                    </font></td>
                </tr>
                <tr> 
                  <td bgcolor="#FFCC99"><i><b><font face="Verdana, Arial, Helvetica, sans-serif" size="1">A&ntilde;o 
                    de la publicaci&oacute;n</font></b></i></td>
                  <td> <font face="Verdana, Arial, Helvetica, sans-serif" size="2"> 
                    <input value="<%=(rsAldea21PublUpd.Fields.Item("yearPublic").Value)%>" type="text" name="txt_year">
                    </font></td>
                </tr>
                <tr> 
                  <td colspan="2"> 
              ...
              <input type="hidden" name="MM_recordId" value="<%= rsAldea21PublUpd.Fields.Item("datosID").Value %>">
            </form>
          </td>
        </tr>
      </table>
...
<%
rsAldea21PublUpdTP.Close();
%>
<%
rsAldea21PublUpd.Close();
%>
