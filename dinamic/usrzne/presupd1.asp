...
<%
var rsAldea21PresCongrUpd__MMColParam = "1";
if(String(Request.Cookies("cLogin")) != "undefined") { 
  rsAldea21PresCongrUpd__MMColParam = String(Request.Cookies("cLogin"));
}
%>
<%
var rsAldea21PresCongrUpd = Server.CreateObject("ADODB.Recordset");
rsAldea21PresCongrUpd.ActiveConnection = MM_connaldea21_STRING;
rsAldea21PresCongrUpd.Source = "SELECT presCongrID, datosPresCongr, yearPresCongr  FROM presCongr  WHERE integID = "+ rsAldea21PresCongrUpd__MMColParam.replace(/'/g, "''") + "  ORDER BY yearPresCongr DESC";
rsAldea21PresCongrUpd.CursorType = 0;
rsAldea21PresCongrUpd.CursorLocation = 2;
rsAldea21PresCongrUpd.LockType = 3;
rsAldea21PresCongrUpd.Open();
var rsAldea21PresCongrUpd_numRows = 0;
%>
<%
var Repeat1__numRows = -1;
var Repeat1__index = 0;
rsAldea21PresCongrUpd_numRows += Repeat1__numRows;
%>
<%
// *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

// set the record count
var rsAldea21PresCongrUpd_total = rsAldea21PresCongrUpd.RecordCount;

// set the number of rows displayed on this page
if (rsAldea21PresCongrUpd_numRows < 0) {            // if repeat region set to all records
  rsAldea21PresCongrUpd_numRows = rsAldea21PresCongrUpd_total;
} else if (rsAldea21PresCongrUpd_numRows == 0) {    // if no repeat regions
  rsAldea21PresCongrUpd_numRows = 1;
}

// set the first and last displayed record
var rsAldea21PresCongrUpd_first = 1;
var rsAldea21PresCongrUpd_last  = rsAldea21PresCongrUpd_first + rsAldea21PresCongrUpd_numRows - 1;

// if we have the correct record count, check the other stats
if (rsAldea21PresCongrUpd_total != -1) {
  rsAldea21PresCongrUpd_numRows = Math.min(rsAldea21PresCongrUpd_numRows, rsAldea21PresCongrUpd_total);
  rsAldea21PresCongrUpd_first   = Math.min(rsAldea21PresCongrUpd_first, rsAldea21PresCongrUpd_total);
  rsAldea21PresCongrUpd_last    = Math.min(rsAldea21PresCongrUpd_last, rsAldea21PresCongrUpd_total);
}
%>
<%
...
if (rsAldea21PresCongrUpd_total == -1) {
  ...
  for (rsAldea21PresCongrUpd_total=0; !rsAldea21PresCongrUpd.EOF; rsAldea21PresCongrUpd.MoveNext()) {
    rsAldea21PresCongrUpd_total++;
  }

  // reset the cursor to the beginning
  if (rsAldea21PresCongrUpd.CursorType > 0) {
    if (!rsAldea21PresCongrUpd.BOF) rsAldea21PresCongrUpd.MoveFirst();
  } else {
    rsAldea21PresCongrUpd.Requery();
  }

  // set the number of rows displayed on this page
  if (rsAldea21PresCongrUpd_numRows < 0 || rsAldea21PresCongrUpd_numRows > rsAldea21PresCongrUpd_total) {
    rsAldea21PresCongrUpd_numRows = rsAldea21PresCongrUpd_total;
  }

  // set the first and last displayed record
  rsAldea21PresCongrUpd_last  = Math.min(rsAldea21PresCongrUpd_first + rsAldea21PresCongrUpd_numRows - 1, rsAldea21PresCongrUpd_total);
  rsAldea21PresCongrUpd_first = Math.min(rsAldea21PresCongrUpd_first, rsAldea21PresCongrUpd_total);
}
%>
<% var MM_paramName = ""; %>
...
<title>PPI Aldea 21 - Actualizar presentaci&oacute;n - 1 de 3</title>
<!-- #EndEditable --> 
        ...
        <tr> 
          <td> 
            ...
            <p><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><i>Seleccione 
              la presentaci&oacute;n que quiere actualizar</i></font></p>
            <p><font face="Verdana, Arial, Helvetica, sans-serif" size="2">Total 
              de Presentaciones: <b><%=(rsAldea21PresCongrUpd_total)%></b> </font></p>
            <table width="100%" border="1">
              ...
              <% while ((Repeat1__numRows-- != 0) && (!rsAldea21PresCongrUpd.EOF)) { %>
              <tr> 
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><%=(rsAldea21PresCongrUpd.Fields.Item("yearPresCongr").Value)%></font></td>
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><b><a href="presupd2.asp?<%= MM_keepNone + ((MM_keepNone!="")?"&":"") + "presCongrID=" + rsAldea21PresCongrUpd.Fields.Item("presCongrID").Value %>"><%=(rsAldea21PresCongrUpd.Fields.Item("datosPresCongr").Value)%></a></b></font></td>
              </tr>
              <%
  Repeat1__index++;
  rsAldea21PresCongrUpd.MoveNext();
}
%>
            </table>
            <p>&nbsp;</p>
