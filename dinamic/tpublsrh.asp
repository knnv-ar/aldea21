...
<%
var reAldea21PublicTipo = Server.CreateObject("ADODB.Recordset");
reAldea21PublicTipo.ActiveConnection = MM_connaldea21_STRING;
reAldea21PublicTipo.Source = "SELECT tipoPublic, public.tipoPublicID  FROM publicTipo, public  WHERE publicTipo.tipoPublicID = public.tipoPublicID  GROUP BY tipoPublic, public.tipoPublicID  ORDER BY public.tipoPublicID";
reAldea21PublicTipo.CursorType = 0;
reAldea21PublicTipo.CursorLocation = 2;
reAldea21PublicTipo.LockType = 3;
reAldea21PublicTipo.Open();
var reAldea21PublicTipo_numRows = 0;
%>
<html>
      ...
      <table width="650" border="0" cellspacing="4" cellpadding="2">
        <tr> 
          <td> 
            ...
            <form name="frm_publTip" method="post" action="tpublrst.asp">
              <table width="100%" border="1">
                <tr> 
                  <td bgcolor="#FFCC99"><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><b><i>B&uacute;squeda 
                    por Tipo de Publicaci&oacute;n</i></b></font></td>
                </tr>
                <tr> 
                  <td> <font face="Verdana, Arial, Helvetica, sans-serif" size="2"> 
                    <select name="pum_ptip">
                      <% 
while (!reAldea21PublicTipo.EOF) {
%>
                      <option value="<%=(reAldea21PublicTipo.Fields.Item("tipoPublicID").Value)%>"><%=(reAldea21PublicTipo.Fields.Item("tipoPublic").Value)%></option>
                      <%
  reAldea21PublicTipo.MoveNext();
}
if (reAldea21PublicTipo.CursorType > 0) {
  if (!reAldea21PublicTipo.BOF) reAldea21PublicTipo.MoveFirst();
} else {
  reAldea21PublicTipo.Requery();
}
%>
                    </select>
