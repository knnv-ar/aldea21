...
<%
var rsAldea21Autor = Server.CreateObject("ADODB.Recordset");
rsAldea21Autor.ActiveConnection = MM_connaldea21_STRING;
rsAldea21Autor.Source = "SELECT integName, public.integID  FROM integ, public  WHERE integ.integID = public.integID  GROUP BY integName, public.integID  ORDER BY integName";
rsAldea21Autor.CursorType = 0;
rsAldea21Autor.CursorLocation = 2;
rsAldea21Autor.LockType = 3;
rsAldea21Autor.Open();
var rsAldea21Autor_numRows = 0;
%>
<html>
  ...
  <tr> 
    <td valign="top"><!-- #BeginEditable "contenido" --> 
      <table width="650" border="0" cellspacing="4" cellpadding="2">
        <tr> 
          <td> 
            <p><b><font size="2" face="Verdana, Arial, Helvetica, sans-serif">PUBLICACIONES</font></b></p>
            <form name="frm_publTip" method="post" action="apublrst.asp">
              <table width="100%" border="1">
                <tr> 
                  <td bgcolor="#FFCC99"><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><i><b><font size="1">B&uacute;squeda 
                    por Autor</font></b></i></font></td>
                </tr>
                <tr> 
                  <td> <font face="Verdana, Arial, Helvetica, sans-serif" size="2"> 
                    <select name="pum_aut">
                      <% 
while (!rsAldea21Autor.EOF) {
%>
                      <option value="<%=(rsAldea21Autor.Fields.Item("integID").Value)%>"><%=(rsAldea21Autor.Fields.Item("integName").Value)%></option>
                      <%
  rsAldea21Autor.MoveNext();
}
if (rsAldea21Autor.CursorType > 0) {
  if (!rsAldea21Autor.BOF) rsAldea21Autor.MoveFirst();
} else {
  rsAldea21Autor.Requery();
}
%>
                    </select>
