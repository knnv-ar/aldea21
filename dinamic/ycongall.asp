...
<%
var rsAldea21Congr = Server.CreateObject("ADODB.Recordset");
rsAldea21Congr.ActiveConnection = MM_connaldea21_STRING;
rsAldea21Congr.Source = "SELECT integName, yearPresCongr, datosPresCongr  FROM presCongr, integ  WHERE integ.integID = presCongr.integID  ORDER BY yearPresCongr DESC";
rsAldea21Congr.CursorType = 0;
rsAldea21Congr.CursorLocation = 2;
rsAldea21Congr.LockType = 3;
rsAldea21Congr.Open();
var rsAldea21Congr_numRows = 0;
%>
<%
var Repeat1__numRows = -1;
var Repeat1__index = 0;
rsAldea21Congr_numRows += Repeat1__numRows;
%>
<%
// *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

// set the record count
var rsAldea21Congr_total = rsAldea21Congr.RecordCount;

// set the number of rows displayed on this page
if (rsAldea21Congr_numRows < 0) {            // if repeat region set to all records
  rsAldea21Congr_numRows = rsAldea21Congr_total;
} else if (rsAldea21Congr_numRows == 0) {    // if no repeat regions
  rsAldea21Congr_numRows = 1;
}

// set the first and last displayed record
var rsAldea21Congr_first = 1;
var rsAldea21Congr_last  = rsAldea21Congr_first + rsAldea21Congr_numRows - 1;

// if we have the correct record count, check the other stats
if (rsAldea21Congr_total != -1) {
  rsAldea21Congr_numRows = Math.min(rsAldea21Congr_numRows, rsAldea21Congr_total);
  rsAldea21Congr_first   = Math.min(rsAldea21Congr_first, rsAldea21Congr_total);
  rsAldea21Congr_last    = Math.min(rsAldea21Congr_last, rsAldea21Congr_total);
}
%>
<%
...
if (rsAldea21Congr_total == -1) {
  ...
  for (rsAldea21Congr_total=0; !rsAldea21Congr.EOF; rsAldea21Congr.MoveNext()) {
    rsAldea21Congr_total++;
  }

  // reset the cursor to the beginning
  if (rsAldea21Congr.CursorType > 0) {
    if (!rsAldea21Congr.BOF) rsAldea21Congr.MoveFirst();
  } else {
    rsAldea21Congr.Requery();
  }

  // set the number of rows displayed on this page
  if (rsAldea21Congr_numRows < 0 || rsAldea21Congr_numRows > rsAldea21Congr_total) {
    rsAldea21Congr_numRows = rsAldea21Congr_total;
  }

  // set the first and last displayed record
  rsAldea21Congr_last  = Math.min(rsAldea21Congr_first + rsAldea21Congr_numRows - 1, rsAldea21Congr_total);
  rsAldea21Congr_first = Math.min(rsAldea21Congr_first, rsAldea21Congr_total);
}
%>
<html>
        ...
        <tr> 
          <td> 
            <p><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><b>PRESENTACIONES 
              A CONGRESOS Y OTROS ORDENADAS POR A&Ntilde;O</b></font></p>
            <p><font face="Verdana, Arial, Helvetica, sans-serif" size="2">Total 
              de presentaciones encontradas: <b><%=(rsAldea21Congr_total)%></b> </font></p>
            <table width="100%" border="1">
              <tr bgcolor="#FFCC99"> 
                ...
                <td> 
                  <div align="center"><b><font size="1"><i><font face="Verdana, Arial, Helvetica, sans-serif">Integrante</font></i></font></b></div>
                </td>
                <td> 
                  <div align="center"><b><font size="1"><i><font face="Verdana, Arial, Helvetica, sans-serif">Detalle</font></i></font></b></div>
                </td>
              </tr>
              <% while ((Repeat1__numRows-- != 0) && (!rsAldea21Congr.EOF)) { %>
              <tr> 
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><%=(rsAldea21Congr.Fields.Item("yearPresCongr").Value)%></font></td>
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><%=(rsAldea21Congr.Fields.Item("integName").Value)%></font></td>
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><%=(rsAldea21Congr.Fields.Item("datosPresCongr").Value)%></font></td>
              </tr>
              <%
  Repeat1__index++;
  rsAldea21Congr.MoveNext();
}
%>
            </table>
          </td>
