...
<%
var rsAldea21PresCongrInteg__MMColParam = "1";
if(String(Request.Form("pum_pci")) != "undefined") { 
  rsAldea21PresCongrInteg__MMColParam = String(Request.Form("pum_pci"));
}
%>
<%
var rsAldea21PresCongrInteg = Server.CreateObject("ADODB.Recordset");
rsAldea21PresCongrInteg.ActiveConnection = MM_connaldea21_STRING;
rsAldea21PresCongrInteg.Source = "SELECT integName, yearPresCongr, datosPresCongr  FROM presCongr, integ  WHERE presCongr.integID = "+ rsAldea21PresCongrInteg__MMColParam.replace(/'/g, "''") + "  AND integ.integID = presCongr.integID  ORDER BY yearPresCongr DESC";
rsAldea21PresCongrInteg.CursorType = 0;
rsAldea21PresCongrInteg.CursorLocation = 2;
rsAldea21PresCongrInteg.LockType = 3;
rsAldea21PresCongrInteg.Open();
var rsAldea21PresCongrInteg_numRows = 0;
%>
<%
var Repeat1__numRows = -1;
var Repeat1__index = 0;
rsAldea21PresCongrInteg_numRows += Repeat1__numRows;
%>
<%
// *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

// set the record count
var rsAldea21PresCongrInteg_total = rsAldea21PresCongrInteg.RecordCount;

// set the number of rows displayed on this page
if (rsAldea21PresCongrInteg_numRows < 0) {            // if repeat region set to all records
  rsAldea21PresCongrInteg_numRows = rsAldea21PresCongrInteg_total;
} else if (rsAldea21PresCongrInteg_numRows == 0) {    // if no repeat regions
  rsAldea21PresCongrInteg_numRows = 1;
}

// set the first and last displayed record
var rsAldea21PresCongrInteg_first = 1;
var rsAldea21PresCongrInteg_last  = rsAldea21PresCongrInteg_first + rsAldea21PresCongrInteg_numRows - 1;

// if we have the correct record count, check the other stats
if (rsAldea21PresCongrInteg_total != -1) {
  rsAldea21PresCongrInteg_numRows = Math.min(rsAldea21PresCongrInteg_numRows, rsAldea21PresCongrInteg_total);
  rsAldea21PresCongrInteg_first   = Math.min(rsAldea21PresCongrInteg_first, rsAldea21PresCongrInteg_total);
  rsAldea21PresCongrInteg_last    = Math.min(rsAldea21PresCongrInteg_last, rsAldea21PresCongrInteg_total);
}
%>
<%
...
if (rsAldea21PresCongrInteg_total == -1) {
  ...
  for (rsAldea21PresCongrInteg_total=0; !rsAldea21PresCongrInteg.EOF; rsAldea21PresCongrInteg.MoveNext()) {
    rsAldea21PresCongrInteg_total++;
  }

  // reset the cursor to the beginning
  if (rsAldea21PresCongrInteg.CursorType > 0) {
    if (!rsAldea21PresCongrInteg.BOF) rsAldea21PresCongrInteg.MoveFirst();
  } else {
    rsAldea21PresCongrInteg.Requery();
  }

  // set the number of rows displayed on this page
  if (rsAldea21PresCongrInteg_numRows < 0 || rsAldea21PresCongrInteg_numRows > rsAldea21PresCongrInteg_total) {
    rsAldea21PresCongrInteg_numRows = rsAldea21PresCongrInteg_total;
  }

  // set the first and last displayed record
  rsAldea21PresCongrInteg_last  = Math.min(rsAldea21PresCongrInteg_first + rsAldea21PresCongrInteg_numRows - 1, rsAldea21PresCongrInteg_total);
  rsAldea21PresCongrInteg_first = Math.min(rsAldea21PresCongrInteg_first, rsAldea21PresCongrInteg_total);
}
%>
<html>
  ...
  <tr> 
    <td valign="top"><!-- #BeginEditable "contenido" --> 
      <table width="650" border="0" cellspacing="4" cellpadding="2">
        <tr> 
          <td> 
            <p><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><b>PRESENTACIONES 
              A CONGRESOS Y OTROS</b></font></p>
            <p><font face="Verdana, Arial, Helvetica, sans-serif" size="2">Total 
              de presentaciones encontradas: <b><%=(rsAldea21PresCongrInteg_total)%></b> </font></p>
            <table width="100%" border="1">
              <tr bgcolor="#FFCC99"> 
                ...
                <td> 
                  <div align="center"><font size="1"><b><i><font face="Verdana, Arial, Helvetica, sans-serif">Integrante</font></i></b></font></div>
                </td>
                <td> 
                  <div align="center"><font size="1"><b><i><font face="Verdana, Arial, Helvetica, sans-serif">Detalle</font></i></b></font></div>
                </td>
              </tr>
              <% while ((Repeat1__numRows-- != 0) && (!rsAldea21PresCongrInteg.EOF)) { %>
              <tr> 
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><%=(rsAldea21PresCongrInteg.Fields.Item("yearPresCongr").Value)%></font></td>
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><%=(rsAldea21PresCongrInteg.Fields.Item("integName").Value)%></font></td>
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><%=(rsAldea21PresCongrInteg.Fields.Item("datosPresCongr").Value)%></font></td>
              </tr>
              <%
  Repeat1__index++;
  rsAldea21PresCongrInteg.MoveNext();
}
%>
            </table>
          </td>
